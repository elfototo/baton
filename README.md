...
from a dump as a fixture, and left behind a test.
...
The generator is deliberately framework-agnostic inside: `core`, `optimize`,
`icons` and `images` know nothing about React, which sits beside them as one
implementation. Adding Vue means adding a folder, not rewriting the pipeline.
...
In active development, being prepared for the Figma Community.
...
Questions, or curious about the implementation? Open an issue or get in touch.
